...
# --- All pods running ---------------------------------------------------------
echo "▸ Pod status"
NOT_READY=$(kubectl get pods -n demo-5g --no-headers 2>/dev/null | grep -v "Running\|Completed" | wc -l | tr -d ' ')
...
for NF in nrf amf smf upf pcf ue-simulator; do
  READY=$(kubectl get deploy "${NF}" -n demo-5g -o jsonpath='{.status.readyReplicas}' 2>/dev/null || echo "0")
  DESIRED=$(kubectl get deploy "${NF}" -n demo-5g -o jsonpath='{.spec.replicas}' 2>/dev/null || echo "?")
  if [[ "${READY}" -ge 1 ]]; then
    echo "  ${PASS} ${NF}: ${READY}/${DESIRED} ready"
  else
    echo "  ${FAIL} ${NF}: ${READY:-0}/${DESIRED} ready"
    errors=$((errors + 1))
  fi
done
echo ""
...
if [[ -n "${NRF_POD}" ]]; then
  HEALTH=$(kubectl exec -n demo-5g "${NRF_POD}" -- python -c "
import urllib.request; print(urllib.request.urlopen('http://localhost:8080/health').read().decode())" 2>/dev/null || echo "failed")
...
if [[ -n "${NRF_POD}" ]]; then
  ...
  if [[ -n "${AMF_POD}" ]]; then
    REG_RESULT=$(kubectl exec -n demo-5g "${AMF_POD}" -- python -c "
...
print(urllib.request.urlopen(req).read().decode())" 2>/dev/null || echo "failed")
...
# --- HPA ----------------------------------------------------------------------
echo "▸ HPA (AMF)"
HPA_EXISTS=$(kubectl get hpa amf -n demo-5g --no-headers 2>/dev/null | wc -l | tr -d ' ')
if [[ "${HPA_EXISTS}" -ge 1 ]]; then
  HPA_TARGETS=$(kubectl get hpa amf -n demo-5g -o jsonpath='{.spec.metrics[0].resource.target.averageUtilization}' 2>/dev/null || echo "?")
...
REGION=$(kubectl config view --minify -o jsonpath='{.clusters[0].cluster.server}' | grep -oP '(?<=\.)\w+-\w+-\d' || echo "us-east-1")
